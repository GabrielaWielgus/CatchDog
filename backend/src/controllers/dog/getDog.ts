...
export const getDog = async (req:Request, res:Response, next:NextFunction) => {
  try{
    ...
    const dogs = await DogRepository.createQueryBuilder('dog')
      .leftJoinAndSelect("dog.treatments", "dogTreatment")
      .leftJoinAndSelect("dogTreatment.treatment", "treatment")
      .leftJoin('dog.owner', 'owner')
      .where("owner.id = :userID", {userID: userID})
      .getMany()


    const resData : GetDogResponse = {
      dogs: dogs
    }
